...
)
{
  ...
  for(devIdx = 0; devIdx < devNum; devIdx++){
    ...
    err = cudaMalloc((void **)&pd_anychange[devIdx], sizeof(int) * blkNum);
    if(err != cudaSuccess)
  ...
  for(i=0; i<numstart; i++){
    ...
    while (changeSum) {//run until no changes
      ...
      for(devIdx=0; devIdx<devNum; devIdx++){
        cudaSetDevice(devIdx);
        err = cudaMemset(pd_anychange[devIdx], 0, sizeof(int) * blkNum);
        if(err != cudaSuccess)
      ...
      for(devIdx=0; devIdx<devNum; devIdx++){
        cudaSetDevice(devIdx);
        err = cudaMemcpyAsync(anychange[devIdx], pd_anychange[devIdx], sizeof(int) * blkNum, cudaMemcpyDeviceToHost, streams[devIdx]);
        if(err != cudaSuccess)
