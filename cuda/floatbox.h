...
//cuda version of all the access methods
__device__ inline extern
size_t
cudaboxindex (
    struct FLOATBOX box,
    int x,
...
// returns a single value from the given coordinates
{
    return box.flat[ cudaboxindex( box, x, y, z ) ];
}
...
// stores a single value at the given coordinates
{
    box.flat[ cudaboxindex( box, x, y, z ) ] = val;
}
...
{
    if( box.flat == NULL ) return;
    size_t i, last = cudaboxindex( box, box.nx-1, box.ny-1, box.nz-1 );
    for( i = 0; i <= last; i++ ) box.flat[i] = val;
}
