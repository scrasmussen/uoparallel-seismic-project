...
__global__ void
cudaWorker(int, int, int, int, int, struct FS *, struct START *, struct VELOCITYBOX *, struct FLOATBOX *,long *);
__device__ int 
...
)
{
  ...
  for(i=0; i<numstart; i++){
    ...
    while (changeSum) {
      ...
      cudaWorker<<<gridDim,blockDim,blkSize*sizeof(float)>>>(
        nx, ny, nz, 
        i, 
        starsize, 
        pd_fs, 
  ...
  //*TODO need to deallocate all the memories.
  cudaFree(pd_fs);
  cudaFree(pd_start);
  cudaFree(pd_vbox);
...
{
  //int d_blktid = threadIdx.z + blockIdx.y * blockDim.z + blockIdx.x * blockDim.z * blockDim.y;
...
) 
{
  ...
  int sx = nz * ny;
	...
	for (l=starstart; l<starstop; l++) {
		...
		delay = dc_fs[l].d * (vboxflat[k+nz*j+i*sx] + vboxflat[ok+nz*oj+oi*sx]) / 2.0;
    ...
    if ((i == startx) && (j == starty) && (k == startz)) {
		...
		tt = ttboxflat[k+nz*j+i*sx];
		tto = ttboxflat[ok+nz*oj+oi*sx];
		/* if offset point has infinity travel time, then update */
		if ((tt == INFINITY) && (tto == INFINITY)) {
			continue;
		}
		if ((tt != INFINITY) && (tto == INFINITY)) {
			ttboxflat[ok+nz*oj+oi*sx] = delay + tt;
			change += 1;
			continue;
		}
		if ((tt == INFINITY) && (tto != INFINITY)) {
			ttboxflat[k+nz*j+i*sx] = delay + tto;
			change += 1;
			continue;
		}
		if ((tt != INFINITY) && (tto != INFINITY)) {
